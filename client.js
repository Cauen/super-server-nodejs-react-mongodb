...
let connectWithSuperServer = () => {
  return ioc.connect(
    "http://" + superServer.ip + ":" + superServer.port + "",
    {
      query: { isUser: true }
    }
  );
...
socketWithSuperServer.on("server", serverIpAndPort => {
  ...
  socketWithServer.on("product_updated", product => console.log(product));
  ...
  socketWithServer.on("connect", async () => {
    ...
    let choose;
    do {
      choose = await ask("Type what do you want - 0: exit, 1: create, 2: read, 3: update, 4: delete ");
      switch(choose) {
        case '1':
  ...
  let listProducts = () =>
    socketWithServer.emit("list_products");
